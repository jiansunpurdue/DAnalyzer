...
HFSequentialVertexFit::HFSequentialVertexFit(Handle<View<Track> > hTracks, const TransientTrackBuilder *TTB, Handle<VertexCollection> pvCollection, const MagneticField *field, BeamSpot beamSpot, int verbose, bool removeCandTracksFromVtx) : 
    fVerbose(verbose),
    fpTTB(TTB),
    fhTracks(hTracks),
...
TAnaCand *HFSequentialVertexFit::addCandidate(HFDecayTree *tree, VertexState *wrtVertexState)
{
    ...
    if (wrtVertexState) {
		...
		if( anaVtx.fD3d/anaVtx.fD3dE < 2.0 && tree->particleID() != 1 ) return pCand;   //decay length cut to reduce time
